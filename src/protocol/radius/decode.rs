...
impl Decoder {
    pub fn new(cs: &custom::Customs) -> Result<Decoder, ()> {
        Ok(Decoder {
            ...
            user_name_col_id:       cs.get(custom::RADIUS_USER_NAME)?,
            acct_status_col_id:     cs.get(custom::RADIUS_ACCT_STATUS)?,
            service_type_col_id:    cs.get(custom::RADIUS_SERVICE_TYPE)?,
            framed_ip_col_id:       cs.get(custom::RADIUS_FRAMED_IP_ADDR)?,
            framed_mask_col_id:     cs.get(custom::RADIUS_FRAMED_IP_MASK)?,
            framed_proto_col_id:    cs.get(custom::RADIUS_FRAMED_PROTO)?,
            acct_session_id_col_id: cs.get(custom::RADIUS_ACCT_SESSION_ID)?,
    ...
    pub fn decode(&mut self, flow: &Flow, cs: &mut custom::Customs) -> bool {
        ...
        if let Some(framed_mask)     = msg.framed_mask      { cs.add_addr(self.framed_mask_col_id, framed_mask); }
